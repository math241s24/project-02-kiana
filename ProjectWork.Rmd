...
fires_county <- fires_county %>%
  ...
  summarise(total_fires_in_county = sum(fires_count), .groups = 'drop') %>%
  mutate(percentage = (total_fires_in_county / total_fires) * 100)

top_counties <- fires_county %>%
...
p <- ggplot() +
  geom_sf(data = ca_sf, aes(geometry = geometry), fill = NA, color = "black") +  
  geom_sf(data = fires_county, aes(fill = percentage), show.legend = FALSE) +  # Here, you color by counties
  geom_sf_text(data = top_counties, aes(label = paste(county_name, " - ", round(percentage, 2), "%")), size = 3, hjust = 1.5) + # and the text is only used for the top 5
  scale_fill_viridis_c(option = "C", begin = 0.2, end = 0.8, name = "Percentage of Fires") +
  labs(title = 'Year: {frame_time}', subtitle = "Top 5 Counties by Fire Proportion") +
  theme_void() +
...
- create maps for of county boundaries colored by these variables
```{r}
...
#^^ this is all present in the fires_county_grp
